...
$Fresh = New-Object System.Collections.Generic.List[double]
foreach ($Range in $FreshRanges) {
    [double]$Min, [double]$Max = $Range -split '-'
    for ($i = $Min; $i -le $Max; $i++) {
        if (-not $Fresh.Contains($i)) { $Fresh.Add($i) }
    }
}

Write-Output "$($Fresh.Count) ingredient IDs are considered to be fresh"
